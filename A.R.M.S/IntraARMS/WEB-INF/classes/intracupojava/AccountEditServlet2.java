 ...
 * @version 1.0
 */
public class AccountEditServlet2 extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if (session.isNew()== true || session.getAttribute("account2")== null)
            account= new AccountInfoBean();
        else if (session.isNew()== false && session.getAttribute("account2")!= null)
            account= (AccountInfoBean)session.getAttribute("account2");
        ...
        {
            query= "DELETE FROM Account_Information WHERE AI_ID='"
            + account.getLogin()+ "'";
            db.update_query(query);
            String temp= "You successfully deleted <b>"+ account.getFirstName()+ " "
                    + account.getLastName()+ "</b>'s account";
        ...
        String temp= "You successfully edited <b>"+ account.getFirstName()+ " "
                    + account.getLastName()+ "</b>'s account";
        session.setAttribute("acceditsuccess", temp);
        response.sendRedirect("action.jsp");
      }
