...
namespace
{
		...
		const eae6320::Graphics::ShaderTypes::eShaderType i_shaderType, const std::string& i_source )
	{
		...
		GLuint shaderId = 0;
		{
			...
			switch ( i_shaderType )
			{
			case eae6320::Graphics::ShaderTypes::eShaderType::Vertex:
				shaderType = GL_VERTEX_SHADER;
				break;
			case eae6320::Graphics::ShaderTypes::eShaderType::Fragment:
				shaderType = GL_FRAGMENT_SHADER;
				break;
			default:
				wereThereErrors = true;
				std::ostringstream errorMessage;
				errorMessage << "No OpenGL implementation exists for the platform-independent shader type " << (int)i_shaderType;
				eae6320::AssetBuild::OutputErrorMessage( errorMessage.str().c_str(), i_path_source );
				goto OnExit;
			}
			// Generate a shader
