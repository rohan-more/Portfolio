...
bool eae6320::AssetBuild::cShaderBuilder::Build( const std::vector<std::string>& i_arguments )
{
	...
	Graphics::ShaderTypes::eShaderType shaderType = Graphics::ShaderTypes::eShaderType::Unknown;
	{
		if ( i_arguments.size() >= 1 )
		{
			...
			if ( argument == "vertex" )
			{
				shaderType = Graphics::ShaderTypes::eShaderType::Vertex;
			}
			else if ( argument == "fragment" )
			{
				shaderType = Graphics::ShaderTypes::eShaderType::Fragment;
			}
			else
			{
